...
사진, 사실 메모, 기존 블로그 글을 바탕으로 네이버 블로그에 옮겨 쓸 **초안**을 만드는 Windows 데스크톱 앱입니다. 자동 발행은 하지 않으며, 작성자가 최종 검토한 뒤 네이버 에디터에 붙여 넣는 방식입니다.

## 주요 기능

- JPG/PNG/GIF/WEBP 사진 선택 및 사진별 메모 입력
- NVIDIA Build/NIM의 OpenAI 호환 API로 사진 분석과 블로그 초안 생성
...
4. 새 글에서 장소, 메뉴, 가격, 운영시간, 방문 계기, 사진 메모처럼 사실을 입력하고 초안을 만든다.
