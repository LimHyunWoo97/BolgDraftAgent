...
앱은 Client Secret을 사용하는 OAuth 인증 코드 방식으로 동작한다. 따라서 Callback URL을 브라우저에 직접 열지 말고, 앱의 **네이버 로그인** 버튼으로 시작해야 한다. 로그인 버튼을 누르기 전에 로컬 콜백 서버를 먼저 열며, 성공 후에는 `로그인 확인이 완료되었습니다` 페이지가 표시된다.
